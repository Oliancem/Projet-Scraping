...
df['Company name'] = df['Company name'].apply(lambda x: re.sub('\s+', ' ', x.strip()))

# Nettoyer la colonne 'Type' en supprimant les informations de contrat et les espaces en double
df['Type'] = df['Type'].apply(lambda x: re.sub('Contract', '', x.strip())) # Supprimer les informations de contrat
df['Type'] = df['Type'].apply(lambda x: re.sub('\s+', ' ', x.strip())) # Supprimer les espaces en double

# Nettoyer la colonne 'Location' en supprimant les informations de région et les espaces en double
df['Location'] = df['Location'].apply(lambda x: re.sub('.*Region', '', x.strip())) # Supprimer les informations de région
df['Location'] = df['Location'].apply(lambda x: re.sub('\s+', ' ', x.strip())) # Supprimer les espaces en double
